...
struct ReusableMessageBuilder: FixBufferStream
{
    ...
    template< typename FIELD, typename VALUE >
    void addConstantHeaderField( const VALUE & v )
    {
        header.append<FIELD>(v);
    }

    template< typename FIELD >
    void addConstantHeaderTag()
    {
        header.pushTag<FIELD>();
    }

    void finalizeConstantHeader()
    {
        header.finalize();
    }
