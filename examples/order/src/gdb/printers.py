        ...
        if val[BUF] != 0:
            for f in self.msg.type.fields():
                if f.type and f.type.name:
                    if f.type.name.startswith( "order::Field" ) and val[f.name][OFFSET] >= 0:
                        self.fields.append( f )
                    if f.type.name.startswith( "order::Group" ) and val[prev.name][OFFSET] >= 0:
                        self.fields.append( f )
                    prev = f
...
def build_fix_printers():
    ...
    pp.add_printer( 'order::MessageHeader', '^order::MessageHeader$', MessagePrinter )
    pp.add_printer( 'order::MessageNewOrderSingle', '^order::MessageNewOrderSingle$', MessagePrinter )
    pp.add_printer( 'order::MessageExecutionReport', '^order::MessageExecutionReport$', MessagePrinter )
    pp.add_printer( 'order::GroupLegStipulations', '^order::GroupLegStipulations$', MessagePrinter )
    pp.add_printer( 'order::GroupLegStipulations::Array', '^std::vector\<order::GroupLegStipulations,.*\>$', GroupPrinter )
    pp.add_printer( 'order::GroupNestedPartySubIDs', '^order::GroupNestedPartySubIDs$', MessagePrinter )
    pp.add_printer( 'order::GroupNestedPartySubIDs::Array', '^std::vector\<order::GroupNestedPartySubIDs,.*\>$', GroupPrinter )
    pp.add_printer( 'order::GroupNestedPartyIDs', '^order::GroupNestedPartyIDs$', MessagePrinter )
