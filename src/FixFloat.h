__COPYRIGHT__
#pragma once
#ifndef DSTHEADERGUARD_FixFloat_h
#define DSTHEADERGUARD_FixFloat_h
...
inline DSTNAMESPACE::Float operator "" _ff( long double q )
{
    return { (double)q };
}
...
{
    return DSTNAMESPACE::Float( ptr );
}

} // namespace DSTNAMESPACE


inline std::ostream & operator << ( std::ostream & os, const DSTNAMESPACE::Float & f )
{
...
inline std::string to_string( const DSTNAMESPACE::Float & f ) noexcept
{
    return f.toString();
}
